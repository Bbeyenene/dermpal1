...
class Member extends Component {
    ...
    handleDelete = product => {
        ...
        if (this.state.savedProducts.map(product => product._id).includes(product._id)) {
            API.deleteProduct(product._id)
                .then(deletedProduct =>
                    this.setState({ savedProducts: this.state.savedProducts.filter(product => product._id !== deletedProduct._id) })
                )
                .catch(err => console.error(err));
        }
                                ...
                                <div className="card col-md-9" >
                                    <div className="row g-0">
                                        <div className="col-md-3">
                                            <img width="200" height="auto" src={data.imag} alt={data.title} />
                                        </div>
                                        ...
                                        <div className="col-md-10">
                                            <div className="myBtn col-md-6">
                                                <button onClick={() => this.handleDelete(data)} className="btn btn-secondary btn-lg"> <svg width="1em" height="1em" viewBox="0 0 16 16" className="bi bi-skip-backward-fill" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
